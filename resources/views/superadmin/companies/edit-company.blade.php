@extends('layouts.app', ['class' => 'g-sidenav-show  bg-gray-100', 'activePage' => 'super-admin', 'folderPage' => trans('labels.navs.superadmin'), 'titlePage' => trans('labels.navs.accounts_devices') . ' / ' . trans('labels.navs_second.edit_company')])
...
@section('content')
    <div class="row mb-8">
        <div class="col-lg-8 mt-lg-0 mt-4 mx-auto my-auto position-relative">
            <div class="card">
                <div class="card-body">
                    <h5 class="font-weight-bolder">@lang('labels.texts.edit_company')</h5>
                    <form action="{{ route('companies.update', $companyData->id) }}" class="form-control" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row mt-4">
                            <div class="col-6">
                                    ...
                                    <select class="selectpicker" id="select-devices" name="devices[]"
                                        ...
                                        @foreach ($tcDevices as $key => $value)
                                            <option value="{{ $value['id'] }}"
                                                {{ in_array($value['id'], $companyData->devices) ? 'selected' : '' }}>
                                                {{ $value['name'] }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-4">
                                <div class="input-group input-group-dynamic">
                                    ...
                                    <select class="selectpicker" id="select-time-zone" name="time_zone"
                                        ...
                                        <option selected value="america/bogota">America/Bogota </option>
                                    </select>
                                    @error('time_zone')
                                        <span class="text-danger text-sm"> {{ $message }}</span>
                                    @enderror
                                    ...
                                    <select class="selectpicker" id="select-permissions" name="permissions[]"
                                        ...
                                        @foreach ($permissions as $key => $value)
                                            <option value="{{ $value['name'] }}"
                                                {{ is_array($companyData->permissions) && in_array($value['name'], $companyData->permissions) ? 'selected' : '' }}>
                                                {{ $value['name'] }}
                                            </option>
                                        @endforeach
                                    </select>
