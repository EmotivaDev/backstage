@extends('layouts.app', ['class' => 'g-sidenav-show  bg-gray-100', 'activePage' => 'super-admin', 'folderPage' => trans('labels.navs.superadmin'), 'titlePage' => trans('labels.navs.accounts_devices') . ' / ' . trans('labels.navs_second.edit_device')])
...
@section('content')
    <div class="row mb-8">
        <div class="col-lg-8 mt-lg-0 mt-4 mx-auto my-auto position-relative">
            <div class="card">
                <div class="card-body">
                    <h5 class="font-weight-bolder">@lang('labels.texts.edit_device')</h5>
                    <form action="{{ route('devices.update', $tcDevices->id) }}" class="form-control" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-6">
                            ...
                            <div class="col-6">
                                <div
                                    ...
                                    <input type="text" name="unique_id"
                                        class="form-control w-100 "value="{{ $tcDevices->uniqueid }}">
                                </div>
                                @error('unique_id')
                                    <span class="text-danger text-sm"> {{ $message }}</span>
                                @enderror
                                ...
                                <div class="input-group input-group-dynamic">
                                    ...
                                    <select class="selectpicker" id="select-type-device" name="type_device"
                                        ...
                                        @foreach ($typeDevices as $key => $value)
                                            <option value="{{ $key }}"
                                                {{ $key === $tcDevices->category ? 'selected' : '' }}>
                                                {{ $value }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="input-group input-group-dynamic">
                                    ...
                                    <select class="selectpicker" id="select-engines" name="engines[]"
                                        ...
                                        @foreach ($engines as $key => $value)
                                            <option value="{{ $key }}"
                                                {{ is_array(json_decode($tcDevices->contact)) && in_array($key, json_decode($tcDevices->contact)) ? 'selected' : '' }}>
                                                {{ $value }}
                                            </option>
                                        @endforeach
                                    </select>
